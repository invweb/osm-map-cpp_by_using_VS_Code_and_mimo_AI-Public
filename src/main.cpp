...
static const float LONG_PRESS_DURATION = 0.5f;
static const float LONG_PRESS_RADIUS = 10.0f;
static const int MAX_QUEUE = 64;
static const int MAX_TEXTURES_PER_FRAME = 8;

static const ImVec4 FLAG_COLORS[] = {
    {0.9f, 0.2f, 0.2f, 1.0f}, {0.2f, 0.7f, 0.2f, 1.0f},
    {0.2f, 0.4f, 0.9f, 1.0f}, {0.9f, 0.7f, 0.1f, 1.0f},
    {0.8f, 0.3f, 0.8f, 1.0f}, {0.1f, 0.8f, 0.8f, 1.0f},
    {0.9f, 0.5f, 0.1f, 1.0f}, {0.5f, 0.2f, 0.9f, 1.0f},
};
static const int FLAG_COLOR_COUNT = sizeof(FLAG_COLORS) / sizeof(FLAG_COLORS[0]);

struct TileRequest { int x, y, zoom; };
struct LoadedTile { int x, y, zoom; std::vector<unsigned char> pixels; int width, height; };
struct TileKey {
    int x, y, zoom;
...
struct Flag { double lat, lon; int color_index; };
...
struct MapApp {
    ...
    void load_flags() {
        ...
        if (!f.is_open()) return;
        flags.clear();
        double lat, lon; int ci;
        while (f >> lat >> lon >> ci) flags.push_back({lat, lon, ci % FLAG_COLOR_COUNT});
        if (!flags.empty()) next_color = (flags.back().color_index + 1) % FLAG_COLOR_COUNT;
    }

    void save_flags() {
        std::ofstream f(flags_path);
        for (auto& fl : flags) f << fl.lat << " " << fl.lon << " " << fl.color_index << "\n";
    }
        ...
        location_thread = std::thread([this]() {
            ...
            if (curl) {
                ...
                curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, +[](char* ptr, size_t sz, size_t nm, void* ud) -> size_t {
                    static_cast<std::string*>(ud)->append(ptr, sz * nm); return sz * nm;
                });
                ...
                if (res == CURLE_OK) {
                    try {
                        ...
                        if (lp != std::string::npos && lop != std::string::npos) {
                            ...
                            location_lat = std::stod(response.substr(lp + 12));
                            location_lon = std::stod(response.substr(lop + 13));
    ...
    void worker_thread() {
        ...
        while (worker_running) {
            TileRequest req{}; bool have = false;
            {
                std::lock_guard<std::mutex> lock(queue_mutex);
                if (!queue.empty()) { req = queue.front(); queue.pop_front(); have = true; }
            }
            if (!have) { std::this_thread::sleep_for(std::chrono::milliseconds(30)); continue; }
    ...
    void request_tile(int x, int y, int z) {
        ...
        {
            std::lock_guard<std::mutex> lock(queue_mutex);
            if ((int)queue.size() >= MAX_QUEUE) return;
        }
        loading.insert(key);
        std::lock_guard<std::mutex> lock(queue_mutex);
    ...
    void clear_tiles() {
        ...
        tiles.clear(); loading.clear();
        std::lock_guard<std::mutex> lock(queue_mutex);
        queue.clear();
    }

    GLuint create_logo() {
        std::vector<unsigned char> px(LOGO_SIZE * LOGO_SIZE * 4);
        for (int y = 0; y < LOGO_SIZE; y++) for (int x = 0; x < LOGO_SIZE; x++) {
            float cx = ((float)x - LOGO_SIZE / 2.0f) / (LOGO_SIZE / 2.0f);
            float cy = ((float)y - LOGO_SIZE / 2.0f) / (LOGO_SIZE / 2.0f);
            float d = sqrtf(cx * cx + cy * cy); int i = (y * LOGO_SIZE + x) * 4;
            if (d < 0.85f) {
                float g = (sinf(cx * 12) * cosf(cy * 12) + 1) * 0.5f, dt = 1 - d * 0.5f;
                px[i] = 20*dt+g*15; px[i+1] = 80*dt+g*40; px[i+2] = 120*dt+g*30; px[i+3] = 255;
            } else if (d < 0.9f) {
                float a = (0.9f - d) / 0.05f * 255;
                px[i] = 40; px[i+1] = 100; px[i+2] = 140; px[i+3] = (unsigned char)a;
            } else { px[i] = px[i+1] = px[i+2] = px[i+3] = 0; }
        }
        GLuint tex; glGenTextures(1, &tex); glBindTexture(GL_TEXTURE_2D, tex);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, LOGO_SIZE, LOGO_SIZE, 0, GL_RGBA, GL_UNSIGNED_BYTE, px.data());
        return tex;
    }
    ...
    void draw_marker(ImDrawList* dl, ImVec2 center, float ts) {
        ...
        dl->AddConvexPolyFilled(pts, 20, IM_COL32(220, 50, 50, 255));
    ...
    void draw_flags(ImDrawList* dl, ImVec2 center, float ts) {
        ...
        for (auto& fl : flags) {
            ...
            ImU32 dark = IM_COL32((int)(c.x*150), (int)(c.y*150), (int)(c.z*150), 255);
            dl->AddCircleFilled(ImVec2(sx+1, sy+2), 8, IM_COL32(0,0,0,50));
            dl->AddCircleFilled(ImVec2(sx, sy), 7, col);
            dl->AddCircle(ImVec2(sx, sy), 7, dark, 0, 1.5f);
    ...
    void init() {
        ...
        flags_path = std::string(home ? home : ".") + "/.osm-map-flags.txt";
        load_flags();
    }

    void process_results() {
        std::deque<LoadedTile> batch;
        bool got_location = false; double loc_lat = 0, loc_lon = 0;
        {
            std::lock_guard<std::mutex> lock(result_mutex);
            batch.swap(results);
            if (location_result_ready) {
                location_result_ready = false; got_location = true;
                loc_lat = location_lat; loc_lon = location_lon;
            }
        }
        int created = 0;
        for (auto& lt : batch) {
            if (created >= MAX_TEXTURES_PER_FRAME) break;
        ...
        if (got_location) {
            ...
            my_lat = loc_lat; my_lon = loc_lon; has_location = true;
            clear_tiles();
        }
    }
    ...
    void update() {
        ...
        if (!splash_done) {
            ...
            if (elapsed >= SPLASH_DURATION) { splash_done = true; }
            else {
        ...
        for (int dy = -ty_count; dy <= ty_count; dy++) {
            for (int dx = -tx_count; dx <= tx_count; dx++) {
                ImVec2 mn(cs.x + dx*TILE_SIZE, cs.y + dy*TILE_SIZE);
                ImVec2 mx(mn.x + TILE_SIZE, mn.y + TILE_SIZE);
                TileKey key{ctx + dx, cty + dy, zoom};
                auto it = tiles.find(key);
                if (it != tiles.end()) dl->AddImage((ImTextureID)(intptr_t)it->second.tex, mn, mx);
                else dl->AddRectFilled(mn, mx, IM_COL32(230,230,230,255));
            }
        }
        ...
        int glfw_btn = glfwGetMouseButton(glfwGetCurrentContext(), GLFW_MOUSE_BUTTON_LEFT);

        if (ImGui::IsMouseClicked(0) && !over_button) {
            ImVec2 mp = io.MousePos;
            if (mp.x > 0 && mp.x < ds.x && mp.y > 0 && mp.y < ds.y) {
                dragging = true; drag_start = mp;
                long_press_active = true; long_press_start = mp;
                long_press_time = glfwGetTime();
                long_press_fired = false;
            }
        }

        if (long_press_active && !long_press_fired) {
            if (glfw_btn == GLFW_RELEASE) {
                long_press_active = false;
            } else {
                ImVec2 mp = io.MousePos;
                float dist = sqrtf((mp.x-long_press_start.x)*(mp.x-long_press_start.x) + (mp.y-long_press_start.y)*(mp.y-long_press_start.y));
                if (dist > LONG_PRESS_RADIUS) { long_press_active = false; }
                else {
        ...
        if (ImGui::IsMouseReleased(0)) { dragging = false; long_press_active = false; }
        ...
        auto draw_btn = [&](float x, float y, const char* label, bool active) {
            ...
            fg->AddRectFilled(mn, mx, active ? IM_COL32(60,60,60,220) : IM_COL32(40,40,40,200), 4.0f);
        ...
        bool mh = io.MousePos.x > bx && io.MousePos.x < bx+bs && io.MousePos.y > by-bs*2-sp*2 && io.MousePos.y < by-bs*2-sp*2+bs;
        bool ph = io.MousePos.x > bx && io.MousePos.x < bx+bs && io.MousePos.y > by-bs-sp && io.MousePos.y < by-bs-sp+bs;
        bool lh = io.MousePos.x > bx && io.MousePos.x < bx+bs && io.MousePos.y > by && io.MousePos.y < by+bs;

        draw_btn(bx, by-bs*2-sp*2, "-", mh);
        draw_btn(bx, by-bs-sp, "+", ph);
        {
            ImVec2 mn(bx,by), mx(bx+bs,by+bs);
            fg->AddRectFilled(mn, mx, lh ? IM_COL32(60,60,60,220) : IM_COL32(40,40,40,200), 4.0f);
            fg->AddRect(mn, mx, IM_COL32(100,100,100,255), 4.0f);
            float cx = bx+bs/2, cy = by+bs/2;
        ...
        if (io.MouseClicked[0]) {
            if (mh && zoom > 1) { zoom--; clear_tiles(); }
            if (ph && zoom < 19) { zoom++; clear_tiles(); }
            if (lh && !location_loading) start_location_request();
        }
...
int main() {
    ...
    glfwDestroyWindow(window); glfwTerminate(); curl_global_cleanup();
    return 0;
}
